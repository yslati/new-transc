...
const LiveGameCard = ({ game }) => {
  ...
  return (
	<div className="mt-10 w-96 h-64 bg-gray-100 hover:bg-gray-200 rounded-xl m-auto hover:scale-110 cursor-pointer transform transition-all flex flex-col shadow-md shadow-gray-300"
	...
	>
		<div className="flex h-full relative gap-2">
			<div className="w-1/2 h-full rounded-l-xl flex flex-col items-center">
				...
				<h2 className="font-semibold tracking-wider font-mono text-green-800 flex items-center">
					<UserIcon className="w-4 h-4 mr-2" />	
					{game.username1}
				</h2>
			</div>

			<h2 className="w-full absolute text-center top-24 text-4xl font-bold text-green-800">VS</h2>
			<div className="w-full gameFont absolute text-center text-xl mt-5 top-36 left-36 font-bold flex space-x-5">
				{<h3>{game.score1}</h3>}
				<h3>:</h3>
				<h3>{game.score2}</h3>
			</div>
		...
		<h2 className="w-full text-center py-1 tracking-wider text-gray-500 font-semibold">
			{ game.type } Pong
		</h2>
	</div>
  )
